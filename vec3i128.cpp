...
namespace UniverseStructure
{
    ...
    public:
        ...
        __m256d ToDouble_Loss()
        {
            ...
            return _mm256_blendv_pd(positiveResult, negativeResult, _mm256_castsi256_pd(upperIsNegative));
        }

        void SetDouble_Loss(const __m256d& value)
        {
            //负值掩码以及取绝对值
            __m256d abs = _mm256_and_pd(value,_mm256_castsi256_pd(_mm256_set1_epi64x(LONG_MAX)));
            //转换高低两部分2的52次方
            __m256d upper2P52_d = _mm256_floor_pd(_mm256_mul_pd(abs, TwoP52_Inv));
            __m256i upper2P52 = double_to_uint64_loss(upper2P52_d);
            __m256d lower2P52_d = _mm256_fnmadd_pd(upper2P52_d, TwoP52, abs);
            __m256i lower2P52 = double_to_uint64_loss(lower2P52_d);
            //将两部分插起来
